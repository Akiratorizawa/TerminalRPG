...
public class Battle extends JFrame implements KeyListener {
    ...
    public Battle(com.dominic.terminalrpg.Window window) {
        ...
        try {
            pokemon = Database.pokemonFetcher();
        ...
        playerSprite = Database.spriteFetcher(String.valueOf(player.id()), "player");
        opponentSprite = Database.spriteFetcher(String.valueOf(opponent.id()), "opponent");
    ...
    public static int damageCalculation(Move move, Pokemon target) {
        ...
        double multiplier = Database.effectiveCheck(move, target);
    ...
    public void enterBattleState(BattleState state) {
        ...
        battleTimer = new Timer(2500, e -> {
            switch (battleState) {
                case PLAYER_MOVE -> {
                    double multiplier = Database.effectiveCheck(lastMove, opponent);
                    playerTurn = true;
                ...
                case OPPONENT_MOVE -> {
                    ...
                    double multiplier = Database.effectiveCheck(lastMove, player);
    ...
    public void moveUsed(Pokemon user) {
        ...
        double multiplier = Database.effectiveCheck(lastMove, target);
